...
class Download {
    ...
    /**
     ...
     canvas = await download.html2canvas(document.getElementById('OutlinePane'))
     download.saveAs('<img src="' + canvas.toDataURL() + '">', '办理单.html')

     canvas.toBlob(function(blob){
        download.saveAs(blob, '办理单.png')
     })
     */
    async html2canvas(element, options) {
        return (__html2canvas__ ? __html2canvas__
            : (__html2canvas__ = await import('html2canvas'))).default(element, options)
    }
}
