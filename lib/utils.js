...
export default {
    ...
    string2Date(dateTime) {
        return Date.prototype.isPrototypeOf(dateTime ? dateTime : (dateTime=new Date())) ? dateTime :
            new Date(isNaN(new Date(dateTime).getTime()) ? new Date(dateTime) : dateTime.replace(/[-T]|[.+a-zA-Z].*/gi,
                c => c === '-' ? '/' : (/^T$/i.test(c) ? ' ' : c.replace(/\.[^\s+:a-zA-Z]*|[:A-Z]/g, ''))
            ))
    },
    isFunction(obj) {
